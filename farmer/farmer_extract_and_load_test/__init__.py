...
def extract(server_name, username, password, form_id, client_pl, phase):

    scto = pysurveycto.SurveyCTOObject(server_name, username, password)
    data = scto.get_form_data(form_id, format = 'csv')
    df = pd.read_csv(StringIO(data))
    df = df[(df['client_pl'] == client_pl) & (df['phase_pl'] == phase)]
...
def transform(root_dir, client_pl, client, df, path):
    ...
    ### Tab 1: Open notes
    open_notes_qc = df.loc[:,["enumerator", "mfid_key", "consent", "primary_resp_name_final","businessname_final", "open_notes"]]
    ...
    duration_qc = duration_qc.merge(duration_e_enum, on='enumerator', how='left')
    
    ### Tab 3: Consent
    consent_qc = df.loc[:,["enumerator", "mfid_key", "consent", "primary_resp_name_final","businessname_final"]]
 ...
 "rate_resp_o", "rate_resp_p", "rate_resp_oh", "rate_resp_a", "rate_resp_k", "enum_info",
  "enum_respanswer", "enum_rightresp"]]
    ...
    df['pts_inventory_yesno'] = np.where(df['pts_inventory'].isnull(), 'N/A', 'Yes')
    ...
    df['ofp_profit_nearestyear_sc'] = np.where(df['ofp_profit_nearestyear'].notna(), 100, 0)
    ...
    df['sales_avg'] = round((df['ofp_valuenearestyear']/df['cms_network'].sum()),1)
    ...
    ### profit trends
    #### convert 0 values to NaN
    cols = ['ofp_profit_nearestyear', 'ofp_profitmiddleyear', 'ofp_profitfurthestyear']
    df[cols] = df[cols].replace({0: np.nan})
    ...
    #### convert 0 values to NaN
    cols = ['ofp_loss_nearestyear', 'ofp_loss_middleyear', 'ofp_loss_furthestyear']
    df[cols] = df[cols].replace({0: np.nan})
    ...
    df['monthscashreserve'] = round(df['ofp_cash_amnt']/df['ofp_monthlyexp'], 1)                                                                                                                                                              
    ...
    if(len(df)>1):
        ...
        df[['cms_categ_variance', 'pts_categ_variance', 'pp_categ_variance', 'rm_categ_variance', 'ofp_categ_variance', 'total_sc_variance']] = 0
    ...
    bd_project_top_scorce_conditions = [(df['bd_ALP_scorce'] == 'Basic Performance'),
                                        (df['bd_ALP_scorce'] =='Moderate Performance'),
    ...
    col_null = ['cms_manager', 'cms_manager_men', 'cms_manager_women', 'cms_empl_unpaid_men','cms_empl_unpaid_women']
    ...
    list_old_columns_yes_no = ALP_labels[ALP_labels['choice_labels']=='yesno_label']['original_name'].unique().tolist()
    list_old_columns_yes_no_99 = ALP_labels[ALP_labels['choice_labels']=='yesno99_label']['original_name'].unique().tolist()
    list_old_columns = ALP_labels['original_name'].unique()
    list_old_columns = list_old_columns[~(np.isin(list_old_columns, list_old_columns_yes_no + list_old_columns_yes_no_99 + ["ofp_borrowed_lender"]))]
    ...
    ### Fill null value
    object_col_not_change_list = ['businessname_final','pts_ledger_update_label', 'pts_bk_how_label', 'pts_inventory_label', 'pts_fs_prep_label', 'cms_credit97']
    non_object_col_not_change_list = ["trucksize_s", "trucksize_m", "trucksize_l", "shed_num_count","trucksize_vl", "trucksize_97", "monthscashreserve"]
    col_object = df.select_dtypes(include=[np.object]).columns
    col_other = df.columns.drop(col_object)
    df.loc[:,col_object.drop(object_col_not_change_list)] = df.loc[:,col_object].fillna('N/A')
    df.loc[:,['ofp_borrowed_issues', 'pts_fs_audit', 'cms_credit_1', 'cms_credit_2', 'cms_credit_97', 'cms_inputcredit_1', 'cms_inputcredit_2']] = df.loc[:,['ofp_borrowed_issues', 'pts_fs_audit', 'cms_credit_1', 'cms_credit_2', 'cms_credit_97', 'cms_inputcredit_1', 'cms_inputcredit_2']].fillna(10000)
    ...
    df['cms_empl_paid_women'] = df['cms_empl_women'] - df['cms_empl_unpaid_women']
    df['cms_empl_paid_condition'] = np.where((df['cms_empl_paid_men']==0) & (df['cms_empl_paid_women']==0), 1, 0)
    df['cms_empl_condition'] = np.where((df['cms_empl_men']==0) & (df['cms_empl_women']==0), 1, 0)
    df['cms_empl_unpaid_condition'] = np.where((df['cms_empl_unpaid_men']==0) & (df['cms_empl_unpaid_women']==0), 1, 0)
    df['cms_manager_condition'] = np.where((df['cms_manager_men']==0) & (df['cms_manager_women']==0), 1, 0)
    df['cms_empl_paid_condition_label'] = np.where(df['cms_empl_paid_condition']==1, 'Not available', '')
    df['cms_empl_condition_label'] = np.where(df['cms_empl_condition']==1, 'Not available', '')
    df['cms_empl_unpaid_condition_label'] = np.where(df['cms_empl_unpaid_condition']==1, 'Not available', '')
    df['cms_manager_condition_label'] = np.where(df['cms_empl_condition_label']==1, 'Not available', '')
    ...
    df['total_rtloyal_network_inputs_topq'] = df['total_rtloyal_network_inputs'].quantile(0.75).round(1)
    df['total_rtloyal_network_product_topq'] = df['total_rtloyal_network_product'].quantile(0.75).round(1)

    df['trucksize_97_label'] = np.where(df['trucksize_97'].isna(), "N/A"
                           ,df['trucksize_97'].round(0).astype('Int64'))
    df['trucksize_s_label'] = np.where(df['trucksize_s'].isna(), "N/A"
                               ,df['trucksize_s'].round(0).astype('Int64'))
    df['trucksize_m_label'] = np.where(df['trucksize_m'].isna(), "N/A"
                               ,df['trucksize_m'].round(0).astype('Int64'))
    df['trucksize_l_label'] = np.where(df['trucksize_l'].isna(), "N/A"
                               ,df['trucksize_l'].round(0).astype('Int64'))
    df['trucksize_vl_label'] = np.where(df['trucksize_vl'].isna(), "N/A"
                               ,df['trucksize_vl'].round(0).astype('Int64'))
    ...
    df['total_warehouses'] = np.where(df['ofp_asset_shed_size_know']==1, df['ofp_asset_shed_size_converted'],"")
    df['total_land_comm'] = np.where(df['land_comm_know']==1, df['land_comm_size_converted'],"")
    df['total_land_ag'] = np.where(df['land_ag_know']==1, df['land_ag_size_converted'],"")
...
def main(mytimer: func.TimerRequest) -> None:
    ...
    transform(root_dir, client_pl, client, df, processed_pre_path)
